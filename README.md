...
- **The debot's opening argument is scored too**, not just its replies to the player. It's generated and self-graded (gain/penalty on the same scale as every later round's `opponent_gain`/`opponent_penalty`) in one call at the start of `startBattle()` (`app/(app)/offline/page.tsx`), and the resulting damage lands on the player before their first turn, with a brief on-screen callout. Previously the opening was pure flavor text with zero score effect — every other line the debot ever says gets judged as part of the following round's judge call, so the opening not being graded at all was really an omission, not a deliberate "player gets a free round" design choice.
...
- **Debot select screen**: `components/arena/DebotStage.tsx` shows a detail panel for whichever debot card was most recently tapped (`viewedId`), independent of which one is actually selected (`selectedOpp`) — so you can freely browse other debots' details without first deselecting the one you've picked. Double-tapping an unlocked card selects it directly, without needing to open its panel and hit the Select button. (Locked cards always just open their panel on any tap/double-tap — you still have to go through Unlock explicitly.)
- **Notifications badge**: the header bell (`components/shell/TopBar.tsx`) polls `lib/notificationsBadge.ts` every 45s for a lightweight boolean — true if there's an incomplete Daily Challenge, a pending friend request, or an undismissed match invite (incoming or outgoing). This intentionally duplicates a little logic from `app/(app)/notifications/page.tsx` rather than sharing it outright, since the page needs the full joined objects (profile names, invite details, etc.) to render and the header only ever needs a yes/no. Nothing here is push/Realtime — it's poll-only, so a new notification can take up to ~45s to show the dot.
...
Unlike `store_items`/`store_themes`, the `debots` table's `id` column is a plain not-null integer with no auto-generating default — that's why creating a debot from the admin panel used to fail with `null value in column "id" violates not-null constraint`. The app now works around this by computing "one past the current highest id" client-side before inserting (see `nextDebotId`/`insertDebotWithRetry` in `AdminPanel.tsx`), retrying against a freshly-read id (up to 3 times) if two admins happen to create a debot at nearly the same moment and collide — so debot creation works without touching the database, and a rare simultaneous-create no longer needs a manual retry by the admin. That retry narrows the race, it doesn't eliminate it (only a real DB-level identity column can do that). If you'd rather fix it properly at the DB level instead:
...
- **Admin panel visual pass covered the shell, not every sub-section.** The outer nav (`AdminPanel.tsx`'s top-level tab grid) was redesigned into a grouped icon grid instead of a wrapping row of pill buttons, and the "Restore default" button was removed from Admin → Learning specifically (the one in Admin → AI → Judge & Scoring is a different, intentionally-kept feature). The individual sub-admin screens themselves (Debots, Topics, Store, etc. — most of this file's ~2700 lines) still use their original, denser form-heavy layout; a full visual pass on those would be a much larger, higher-risk follow-up.
- **Achievements can't be freely invented from the admin panel** — Admin → Achievements only lets you tweak/reorder/retire the existing catalog (rewards, icons, active toggle, tiers), not create a brand new one. Every achievement's `condition_type` is a fixed enum the game's own achievement-checking code (`lib/achievements.ts`) knows how to evaluate (total wins, items used, specific debot beaten, etc.) — a "new" achievement created purely through the admin UI would just silently never unlock for anyone, since there's no way to define arbitrary new unlock logic without writing the matching code. Same category of issue as the store-items note above (a new store item `key` needing real game-logic hooked up before it does anything) — the "+ New Achievement" button was removed for exactly that reason, rather than leaving an admin control that looks functional but produces achievements nobody can ever earn.
- **Daily Challenge "already completed" now caches locally, for everyone.** `hasCompletedToday()` (`lib/dailyChallengeStatus.ts`) and the Daily Challenge screen's own completion check used to rely solely on a live `daily_challenge_attempts` SELECT for logged-in users on every load — if that read didn't come back in time (RLS, session-not-ready-yet, a network hiccup, whatever the specific cause), the screen would show the quiz again after a refresh even though the user had already submitted, and re-submitting then failed with "Already completed" since the DB row genuinely existed. Both places now check a local cache first (written right after a successful submit, previously guest-only) before ever touching the DB, so a same-browser refresh is now instant and can't regress to "not done" — a brand-new device/browser with no local cache still needs that DB read to know, same as before.
- **The header notification dot no longer sticks on "Daily Challenge incomplete" forever.** It shares the same `hasCompletedToday()` check above, so the local-cache fix also fixes this — the dot used to stay lit even after completing, for the same underlying reason.
- **Reasoning models (GPT-OSS, Qwen3) need `reasoning_effort` set, or JSON-parsing routes silently break.** All three routes that call Groq directly (`app/api/debate`, `app/api/daily-challenge`, `app/api/online/score-turn`) now send `reasoning_effort: "low"` when the configured model is GPT-OSS or Qwen3 (see `lib/groqReasoning.ts`), and every place that parses a JSON response from the model (`extractJSON`/`extractJSONArray`) now strips a `<think>...</think>` block first if present. Without this, switching the admin-configured model to a reasoning model (e.g. after `llama-3.3-70b-versatile`'s Aug 2026 decommission) causes every route that returns free-form text (debot replies, ace-card hints) to keep working normally, while every route that returns strict JSON (the round judge, PvP scoring, Daily Challenge question generation) silently breaks — the model's internal reasoning either leaks into the same field the code parses, or eats into the `max_tokens` budget before the real JSON gets written, and the resulting `JSON.parse()` failure gets caught and defaults to a zeroed-out/fallback result with no visible error. `config/AI.ts`'s code-level fallback model was updated to `openai/gpt-oss-120b` for the same decommission reason — see the environment variables section above for how that interacts with the admin-configured model.
